...
from Test_Case import TestCaseIncar2


class ErrorCalIncar2:
    ...
    def error_cal(self):
        TestCaseIncar2.tc2_obj.impl_incr_2()
        for i in range(0, 5):
            func_def_obj_i_2.round_out = True
            self.absolute_err.append(func_def_obj_i_2.round_off_val(
                self.actual_value[i] - TestCaseIncar2.tc2_obj.length_data[i]))
        ...
        for j in range(0, 5):
            print("Radius: ", "Computed Length", "\t", "Expected Length", "\t",
                  "Absolute error: ", "\t", "Relative error:")
            print(j + 1, "\t\t", TestCaseIncar2.tc2_obj.length_data[j], "\t\t", self.actual_value[j], "\t\t",
                  self.absolute_err[j], "         \t\t", self.relative_err[j])


error_cal_incar_2_obj = ErrorCalIncar2()
